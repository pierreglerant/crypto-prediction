"""Airflow DAG for GDELT media pipeline on Ethereum (bronze -> silver -> gold)."""
...
def _load_source_mappings() -> dict[str, str] | None:
    ...
    raw = _get_variable("GDELT_ETH_SOURCE_MAPPINGS_JSON", "")
    if not raw:
        return None

    parsed = json.loads(raw)
    if not isinstance(parsed, dict):
        raise ValueError("GDELT_ETH_SOURCE_MAPPINGS_JSON must be a JSON object")
...
def task_bronze(**context):
    ...
    coin_name = _get_variable("GDELT_ETH_COIN", "ethereum").strip().lower()
    query_terms = _parse_query_terms(_get_variable("GDELT_ETH_QUERY_TERMS", "ethereum,eth"))
    fetch_missing = _get_variable("GDELT_ETH_FETCH_MISSING", "false").lower() == "true"
...
with DAG(
    dag_id="gdelt_media_eth_pipeline",
